...
// 工具函數
const calculateCorrectTotal = (order) => {
...
const orderStats = computed(() => {
  const ordersData = ordersArray.value;
  const total = ordersData.length;
  const paid = ordersData.filter((order) => order.is_paid).length;
  const unpaid = total - paid;
  const totalAmount = ordersData.reduce((sum, order) => sum + calculateCorrectTotal(order), 0);
...
// API 方法
const getOrders = async (currentPageNum = 1) => {
  currentPage.value = currentPageNum;
  const url = `${VITE_APP_URL}/api/${VITE_APP_PATH}/admin/orders?page=${currentPageNum}`;
...
const updatePaid = async (item) => {
  ...
  try {
    const response = await axios.put(url, { data: paid });
